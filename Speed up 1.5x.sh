...
main() {
	...
	local new_file="${input_file%.*}_1.5x.mp4"
	local wip_summ="Speeding up 1.5x"
	local wip_body="Processing: ${input_file##*/}"
	local success_summ="Speed up complete"
	local success_body="Saved: ${new_file##*/}"
	
	if [ "$input_file" ]; then
		notify-send "$wip_summ" "$wip_body" -t $timeout &
		local error_msg=$(ffmpeg -i "$input_file" \
			-vf "setpts=0.66666*PTS,pad=ceil(iw/2)*2:ceil(ih/2)*2,scale=trunc(iw/2)*2:trunc(ih/2)*2" \
			-filter:a "atempo=1.5" \
